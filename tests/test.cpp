...
TEST_CASE("F functions")
{
    ...
    SECTION("F2")
    {
        ...
        for (auto kv : buckets) {
            ...
            int32_t idx_count = f2.FindMatchesIdx(left_bucket, right_bucket, idx_L, idx_R);
...
TEST_CASE("Sort on disk")
{
    ...
    SECTION("Sort in Memory")
    {
        ...
        UniformSort::SortToMemory(disk, begin, memory.get(), size, iters, Util::RoundSize(iters), 16);
