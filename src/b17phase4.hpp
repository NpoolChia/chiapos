...
void b17RunPhase4(uint8_t k, uint8_t pos_size, FileDisk &tmp2_disk, b17Phase3Results &res, const bool show_progress, const int max_phase4_progress_updates)
{
    ...
    uint64_t begin_byte_C2 = begin_byte_C1 + (total_C1_entries + 1) * (byte_align / 8);
    uint64_t total_C2_entries = cdiv(total_C1_entries, kCheckpoint2Interval);
    uint64_t begin_byte_C3 = begin_byte_C2 + (total_C2_entries + 1) * (byte_align / 8);
    ...
    auto C1_entry_buf = new uint8_t[byte_align / 8];
    ...
    for (uint64_t f7_position = 0; f7_position < res.final_entries_written; f7_position++) {
        ...
        if (f7_position % kCheckpoint1Interval == 0) {
            ...
            tmp2_disk.Write(final_file_writer_1, (C1_entry_buf), byte_align / 8);
            final_file_writer_1 += byte_align / 8;
            if (num_C1_entries > 0) {
    ...
    Bits(0, byte_align).ToBytes(C1_entry_buf);
    tmp2_disk.Write(final_file_writer_1, (C1_entry_buf), byte_align / 8);
    final_file_writer_1 += byte_align / 8;
